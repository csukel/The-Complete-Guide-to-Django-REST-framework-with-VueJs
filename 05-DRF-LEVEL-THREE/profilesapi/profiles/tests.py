...
class ProfileViewSetTestCase(APITestCase):
    ...
    def test_profile_update_by_owner(self):
        ...
        self.assertEqual(json.loads(response.content),
                        {"id":1,"user":"davinci","bio":"Genius","city":"Nicosia","avatar":None})
...
class ProfileStatusViewSetTestCase(APITestCase):
    ...
    def test_status_update_by_owner(self):
        data = {'status_content':"a new status! (Updated)"}
        response = self.client.put(reverse('status-detail', kwargs={"pk":1}),data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['user_profile'],'davinci')
        self.assertEqual(response.data['status_content'],'a new status! (Updated)')
    ...
    def test_status_update_by_random_user(self):
        ...
        data = {'status_content':"a new status! (HACKED!!)"}
        response = self.client.put(reverse('status-detail', kwargs={"pk":1}),data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)